...
We assume that $Gpt(\pi)$ is a linear map to the true reward function $R_{\text{true}}(\pi)$, where $Gpt$ is the VLM-based reward function. Let $R_{\text{proxy}}(\pi)$ be a proxy for the reward function. Let $\mu$ and $\sigma$ represent changes in proxy reward across time steps. We define the threshold for reward hacking as:

\begin{equation}
    \epsilon = \operatorname{Jensen-Shannon}\big(Gpt(\pi) - R_{\text{proxy}}(\pi, \theta)\big)
\end{equation}

where $\theta$ is a hyperparameter that controls the confidence interval at which the divergence in reward is significant.
